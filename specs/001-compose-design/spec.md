...
- 環境定義に記載された基盤ミドルウェア、プラットフォーム本体、SaaS 層、下流マイクロサービス層を 1 つの `compose.yaml` に落とし込める構成案を定義する。
- 各 service の `depends_on`、`healthcheck`、環境変数、公開ポート、初期化ジョブを含む設計表を定義する。
- 単一の `compose.yaml` で、起動順、依存関係、観測系、永続化、任意起動の扱いまで判断できる状態にする。

### 非目的

- 各 service の実装そのものを開発すること。
- Dockerfile やアプリケーションコードをこの spec で変更すること。
- 本番向けのクラスタ運用やマルチノード構成を決めること。
...
開発者は、基盤ミドルウェア、プラットフォーム本体、SaaS 層、下流マイクロサービス層を 1 つの `compose.yaml` で起動したい。これにより、環境構築の手順を一本化できる。
...
- ワークフロー定義: この feature では対象外
- アクティビティ参照: この feature では対象外
- ランタイム登録手順: この feature では対象外
...
- プラットフォーム側デプロイの要否: `compose.yaml` の構成案として、アプリケーション再デプロイではなく compose 設定の更新で反映できることを前提とする
...
- mail-service または mail-service-mock が未実装の場合の扱い。
...
- mail-service は実装がないため、必要なら mock として compose 案に含める。
